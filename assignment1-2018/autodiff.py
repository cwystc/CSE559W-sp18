...
class MatMulOp(Op):
    ...
    def gradient(self, node, output_grad):
        ...
        A = node.inputs[0]
        B = node.inputs[1]
        return [node.op(output_grad, B, trans_A= (False != node.matmul_attr_trans_A), trans_B=(True != node.matmul_attr_trans_B)), node.op(A, output_grad, trans_A=(True != node.matmul_attr_trans_A), trans_B=(False != node.matmul_attr_trans_B))]
...
def gradients(output_node, node_list):
    ...
    for node in reverse_topo_order:
        ...
        if (len(node.inputs)>0):
            t = node_to_output_grad[node]
            tt = node.op.gradient(node,t)
            for i,n1 in enumerate(node.inputs):
                if (node_to_output_grads_list.get(n1)==None):
                    node_to_output_grads_list[n1]=[tt[i]]
                else:
                    node_to_output_grads_list[n1].append(tt[i])
